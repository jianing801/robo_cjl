...
| 碰撞盒 | origin.z和size.z按长度比例缩放 | 固定近端位置和远端间隙，只改变中间盒体的z向长度；非盒或旋转盒显式报错 |
| 网格路径 | 固定目录软链接 | 根据模板位置或--mesh-dir解析实际路径 |

更新的link为left/right_thigh_pitch_link与left/right_knee_link。其他link和关节参数保留（mesh文件路径统一改为绝对路径）。保留原关节origin的x、y和rpy、轴、限位等。视觉STL不随长度重新建模，与旧生成器的行为一致。碰撞盒只覆盖中间连杆：从原始模板计算盒体上端相对近端关节的位置和盒体下端到远端关节的间隙，改变腿长时固定这两个端部区域，只增加或缩短中间盒体的z向长度。

碰撞盒更新不是整体等比缩放。设原盒中心为z0、高度为h0、原腿长为L0，新腿长为L，则先从原始模板计算：

- 固定上边界：z_top=z0+h0/2；
- 固定远端间隙：d=(-L0)-(z0-h0/2)的绝对间距，代码等价保存为(z0-h0/2)+L0；
- 新下边界：z_bottom=-L+d；
- 新高度：h=z_top-z_bottom；
- 新中心：z=(z_top+z_bottom)/2。

当前模板对应的大腿固定上边界为0.03m、膝端间隙为0.06m；小腿固定上边界为-0.02m、踝端间隙为0.04m。x、y原点和尺寸保持不变。生成报告的每个link会记录collision_boxes，包括实际origin、size、上下边界和远端间隙。

新脚本不会用几何缩放比例再次乘以拟合质量或惯量。输出固定为新目录的urdf/rpo.urdf，附rpo.report.json记录实际写入值、坐标映射和外推状态。拒绝覆盖输入模板或既有输出，避免误改源模型。每次使用新的output目录。

## 必须明确的坐标关系

DM-J10010L-2EC、RS04、RS06和DM-J4340P-2EC的当前报告坐标均与左侧对应link对齐，因此默认使用`Q=I`、`t=0`直接写入左侧。RS04已在2026-09-19根据修正坐标系重新导出并重新拟合，旧版装配体坐标旋转和平移已移除。右侧继续采用左右结构对称假设，沿y方向镜像。
...
- PD刚度、阻尼和延迟保持原值；
- 关节位置随腿长更新；碰撞盒固定两端安装间隙，只改变中间连杆长度；视觉网格仍未针对每个电机和长度重新导出。
...
- 关节z正确，碰撞盒两端间隙保持且中间长度更新，非目标link/关节参数保持，源模板哈希不变。
- 输入范围和已有输出保护生效。
...
视觉网格不随腿长改变，关节位置随长度更新，碰撞盒固定两端安装区、只改变中间连杆长度。动力学采用inertial，接触采用collision；视觉模型可影响渲染和视觉传感器输入。Isaac导入后应实际核对质量/惯量被采用、碰撞几何正确，不能仅凭源配置未出现覆盖选项推断所有导入默认值。
